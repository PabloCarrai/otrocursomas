...
from video472 import Ui_InicioSesion
...
class AplicacionInicioSesion(QMainWindow):
    ...
    def iniciar_sesion(self):
        ...
        if len(nombre_usuario) and len(contrasegnia):
            if nombre_usuario == "pablo" and contrasegnia == "pabl0":
                mensaje.setIcon(QMessageBox.Information)
                mensaje.setText("Bienvenido")
            else:
                mensaje.setIcon(QMessageBox.Warning)
                mensaje.setText("Credenciales escritas invalidas")

                pass
        else:
            mensaje.setIcon(QMessageBox.Warning)
            mensaje.setText("Todos los campos son obligatorio")
